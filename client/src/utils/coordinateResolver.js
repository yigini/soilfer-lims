/**
 * Coordinate and Location Provenance Resolver (Client)
...
export function extractNumeric(val) {
...
/**
 * Resolve coordinates and provenance from a sample record or form state.
 * Accepts sampleData object or raw fieldMetadata/sampling object.
 */
export function resolveCoordinates(sample) {
    if (!sample) {
        return {
            status: 'NOT_LOADED',
            lat: null,
    ...
    // 1. Explicit sample fields (database columns or normalized payload)
    ...
    // 4. Fallback to sampling.coordinates if explicitly provided on sample object
    if ((lat === null || lng === null) && sample.coordinates && typeof sample.coordinates === 'object') {
        const sLat = extractNumeric(sample.coordinates.lat ?? sample.coordinates.latitude);
        const sLng = extractNumeric(sample.coordinates.lng ?? sample.coordinates.longitude);
        if (isValidLat(sLat) && isValidLng(sLng)) {
            lat = sLat;
            lng = sLng;
            accuracy = extractNumeric(sample.coordinates.accuracy ?? sample.positionalUncertaintyM);
            elevation = extractNumeric(sample.coordinates.elevation);
            source = sample.locationSource || 'DESK_CAPTURE';
        }
    }
