        ...
        <div className="flex flex-col h-full gap-4 pb-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                    <Monitor className="text-blue-600 dark:text-blue-400" size={24} /> Equipment Registry
                </h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div className="p-3 rounded-xl card-base border flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-gray-500">Blocked / Overdue</div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, tag, SN..."
            ...
            <div className="card-base rounded-xl shadow-sm border flex-1 overflow-auto">
                {loading ? (
                    <div className="flex items-center justify-center h-48 text-gray-400"><RefreshCw size={24} className="animate-spin mr-2" /> Loading registry...</div>
                ) : filteredAssets.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                        <Monitor size={40} className="mb-2 opacity-40" />
                        <p className="text-sm">No instruments found matching filters</p>
                    </div>
                ) : (
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700/60 border-b dark:border-gray-600 sticky top-0">
                            <tr>
                                <th className="px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Instrument</th>
                                <th className="px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Specification</th>
                                <th className="px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase text-center">Status</th>
                                <th className="px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase text-center">Readiness</th>
                                <th className="px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Last Calib.</th>
                                ...
                                <tr key={asset.id} onClick={() => setSelectedAsset(asset)} className="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 cursor-pointer transition-colors group">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="text-xs text-gray-600 dark:text-gray-400">
                                            {asset.qualification?.lastCalibrationDate ? new Date(asset.qualification.lastCalibrationDate).toLocaleDateString() : 'Never'}
                                        </div>
                                        <div className="text-[10px] text-amber-600 font-medium">
                                            Next: {asset.qualification?.nextCalibrationDueDate ? new Date(asset.qualification.nextCalibrationDueDate).toLocaleDateString() : 'Unscheduled'}
                                        </div>
                                    </td>
