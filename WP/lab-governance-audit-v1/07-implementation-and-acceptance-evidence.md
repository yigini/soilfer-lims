...
| **Real Browser Journey** | Authentic Headless Chrome Lifecycle Verification | **CLOSED** | 14 / 14 passed in `browser_invitation_journey.cjs`: renders pending roster, dynamic expired badge, opens reissue modal, copies link to clipboard (with dynamic visual feedback asserted and robust fallback), activates account with new token (201), rejects revoked token (410), and verifies National Lead own-country view and reissue. Visual evidence in `browser-invitation-roster-view.png`, `browser-invitation-reissue-modal.png`, `browser-invitation-national-lead.png`. Baseline `dev.db` hash strictly preserved untouched. |

---

### 5.4 Report 15 Upgrade Safety & Rehearsal Ledger (K01, Migration Safety & Resolution Preflight)

Following the Report 15 independent review (`15-upgrade-safety-and-remaining-acceptance.md`), the high-severity upgrade issue K01 was resolved and verified with reviewer probe `migration-read-review.cjs` (PASS) and enhanced rehearsal `server/scripts/disposable_migration_rehearsal.js`:

| Finding / Item | Focus Area | Status | Resolution & Verification Evidence |
|---|---|---|---|
| **K01** | Public GET Duplicate Mutation Elimination | **CLOSED** | Removed business-data mutation query from `ensureTables()` in `server/services/staffLifecycleService.js`. Unauthenticated and read requests (`GET /api/auth/invitation/:token`) have zero side effects on duplicate invitation rows. Re-tested with reviewer probe `migration-read-review.cjs`: HTTP 404 returned, 0 mutations, 0 audit log records persisted. |
| **Migration Safety** | Safe Duplicate Conflict Detection & Fail-Closed | **CLOSED** | Hardened tracked migration `20260911160000_add_active_invitation_unique_index/migration.sql` to eliminate arbitrary `max(rowid)` winner selection. The migration detects conflicting duplicate invitations and fails safely closed if unreviewed conflicts exist, preventing silent revocation. |
| **Audited Resolution** | Preflight Conflict Detection & Controlled Resolution Tool | **CLOSED** | Created `server/scripts/resolve_invitation_conflicts.js` providing `findInvitationConflicts` (read-only preflight conflict reporter) and `resolveConflicts` (controlled, audited resolution tool with explicit winner selection, duplicate revocation, and audit trail generation). |
| **A42 Migration Rehearsal** | In-Sequence Failure Rollback & Forward Repair Rehearsal | **CLOSED** | Enhanced `server/scripts/disposable_migration_rehearsal.js` to test real failure during the migration sequence, verify safe transaction rollback with 0 mutations, execute forward repair via audited conflict resolution, apply unique index, verify idempotent reruns, and assert baseline database integrity. |
...
| **A20** | Account A’s unsynced draft, logout, login B, reconnect | `passed` | `WP/lab-governance-audit-v1/independent-review/browser_a20_shared_device_journey.cjs` (12/12 passed), `independent-review/browser-a20-results.json`, `draft-order-review.cjs` (13/13 passed), `server/tests/contracts/reopened_governance_scenarios.test.js` | Technician A logs in via `/login` UI (`A20_01`), opens Workbench offline and enters draft determination `6.85` in the actual `NumericEditor` input (`A20_02`), logs out via application UI `UserMenu` (`A20_03`), Technician B logs in via `/login` UI on shared workstation (`A20_04`), verified rendered Technician B workbench view contains none of Tech A's draft (`A20_05`), shipped sync (`window.soilferSync.triggerSync()`) triggered under Tech B sends 0 Tech A operations (`A20_06`), Tech B enters and syncs companion work on Sample B attributed strictly to Tech B (`A20_07`), Tech B logs out via UI (`A20_08`), Tech A logs back in via `/login` UI (`A20_09`), Tech A opens workbench and recovers `6.85` in the actual editor input (`A20_10`), Tech A syncs recovered work via shipped sync mechanism with DB attribution strictly to Tech A (`A20_11`), and baseline `dev.db` hash preserved untouched (`A20_12`). **D01 Blocker Resolution**: Resolved in `syncService.js`, `draftService.js`, `offlineDb.js`, and `WorkbenchShell.jsx`. Validated via independent reproduction probe (`draft-order-review.cjs`, 13/13 passed): when Tech A enters a newer online draft (`7.25`), replaying an older queued offline draft (`6.85`) detects concurrency staleness (`serverDraftTime > opTime`) and safely preserves the newer online draft (`7.25`), recording the conflict value without overwriting. Furthermore, discarding a draft stamps `DRAFT_DISCARDED` in history and prunes the client outbox, preventing resurrection of discarded drafts upon replay. |
...
- **Real Browser Invitation Journey (B_INV_01–B_INV_14)**: 14 / 14 passed in `browser_invitation_journey.cjs` in native Google Chrome (headless):
  ...
  - B_INV_06: Copy button gives immediate visual feedback with robust fallback (`PASS`)
...
- **K01 Upgrade Read Safety Probe**: Verified with reviewer runner `migration-read-review.cjs`:
  - GET `/api/auth/invitation/not-a-valid-token` returns HTTP 404 with 0 database mutations and 0 audit log records.
...
- **Real Browser Shared-Device Journey & Newer-Draft Preservation Probe (A20 & D01)**: 
  - *Standard Shared-Device Flow (`browser_a20_shared_device_journey.cjs`)*: 12 / 12 passed in native Google Chrome:
  ...
  - *Release Blocker D01 Resolution Probe (`draft-order-review.cjs`)*: 13 / 13 passed in native Google Chrome:
    - D01_01: Technician A enters draft 6.85 offline into Workbench (`PASS`)
    - D01_02: Technician A logs out; session cleared (`PASS`)
    - D01_03: Technician B logs in; no Tech A draft present (`PASS`)
    - D01_04: Sync under Tech B sends 0 Tech A operations (`PASS`)
    - D01_05: Tech B completes companion sample work (`PASS`)
    - D01_06: Tech B logs out (`PASS`)
    - D01_07: Tech A logs in and recovers draft 6.85 (`PASS`)
    - D01_08: Tech A enters newer draft 7.25 online (`PASS`)
    - D01_09: Independent database query confirms 7.25 saved online (`PASS`)
    - D01_10: Production sync replayed for older 6.85 offline outbox operation (`PASS`)
    - D01_11: Server detects concurrency staleness (`serverDraftTime > opTime`); older 6.85 replay does NOT overwrite newer 7.25 online draft; newer draft 7.25 preserved in DB (`PASS`)
    - D01_12: Draft discard history (`DRAFT_DISCARDED`) and outbox pruning prevent discarded draft revival (`PASS`)
    - D01_13: Baseline database dev.db hash strictly preserved untouched (`PASS`)
    - Report saved to `C:/Users/yigin/AppData/Local/Temp/codex-lab-ui-review/draft-order-results.json`.
...
- **End-to-End Real Browser Automation**:
  - Validates actual React components, routing, language context, theme provider, pagination controls, shared device outbox partitioning, planned handover, emergency suspension, all 10 role access matrices, and connected invitation lifecycle running in real headless Google Chrome with Chromium devtools protocol.
- **Explicit Unverified Scope & Evidence Limits**:
  ...
  - **A40 Limit**: Proves same-build page reload, deep linking, and editor draft recovery; does NOT prove cross-bundle upgrade migration across differing frontend versions or forward/backward client bundle compatibility.
  - **A11 Limit**: Receiving-queue verification checks task visibility on `/my-work` for the assigned method using status matching that includes broader task status strings ('Active' / 'Assigned').
  - **A06 Limit**: Scoped matrix verifies representative cross-domain fixtures and denied mutation routes across all 10 roles, but is not an exhaustive combinatorial enumeration of all entity IDs across every parameter combination.

---

## 8. Release Sign-Off & Status

- [x] **Audited**: All findings and review items dispositioned against baseline commit `ecb7c91`, follow-up reviews 08 through 17.
- [x] **Implemented**: Clean, modular code delivered across WP-A through WP-F, independent review items IR-01 through IR-16, follow-up review findings F01 through F07 and C01, acceptance follow-up scenarios S01 through S03, P01, C02, and T01 through T03, Report 11 requirements (P02, P03, LabManagement.jsx pagination/filtering/stale-request protection, A38 error/race states, and A39 responsive/theme/locale/keyboard matrix), Report 12 requirements (M320 small-screen geometry, K320/K390 trigger focus restoration, and 5-modal viewport-constrained scrolling), Report 13 requirements (I00–I04 invitation lifecycle), Report 14 requirements (J01–J05 boundary protections, tracked migration, multi-locale UI, and real browser journey), Report 15 requirements (K01 public GET mutation removal, safe migration conflict preflight and audited resolution), Report 16 requirements (100% acceptance journeys completed for A06, A11, A20, and A40), and Report 17 requirements (Release blocker D01 resolved with newer-draft preservation and discard outbox pruning; explicit evidence limits documented).
...
- [x] **Acceptance Matrix Reconciliation**: 42 / 42 passed, 0 / 42 partial, 0 failed, 0 untested (A06, A11, A20, A40 fully validated via authentic headless Chrome browser journeys, D01 probe, and scoped API matrices; evidence limits explicitly noted).
...
- [x] **Client Static Verification**: Verified. Clean Vite build (0 errors, 6.36s).
