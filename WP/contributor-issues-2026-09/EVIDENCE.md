...
| **#119** | 1 | Confusion between canonical, lab, and original IDs; 26-task fixture truncation; derived fractions vs ordered tasks; ReferenceError: ArrowRight is not defined on manager assignment route | Reproduced & Implemented Locally | `1ed46ef` + follow-up | `sample_assignment_identity.test.js` (8/8), `sample_assignment_manager_route.test.js` (10/10), client production build (`vite build` clean in 7.02s), isolated Headless Chrome CDP browser journey (7/7 steps passed with real DOM clicks, `sample_detail_manager_assign_evidence.json`) | `7516f0e` | Disambiguated canonical UUID (`sampleId`), sample lab ID (`labId`), and field ID (`originalId`); honest task pagination (26-task fixture exposes all 26 tasks with honest pagination metadata); bidirectional texture equivalence (`TEXTURE` satisfied by 3 fractions `SAND, SILT, CLAY`, and composite `TEXTURE` satisfies fractions); independent gate evaluation respecting SKIPPED/WAIVED; sampleWorkspace extracts linked QC batches to gate final approval; unassignedCount calculated across all work items; ManagerQueue routes assign to `?tab=work` and review to `?tab=review`. Prioritized canonical ID lookups (`findUnique({ where: { id } })`) before labId/originalId aliases to prevent cross-column collisions. Resolved live #119 blocker on deployed `7516f0e`: imported missing `ArrowRight` and `Eye` from `lucide-react` in `SampleDetail.jsx`; suite-isolated fixtures (`S005-ROUTE`, `S004-ROUTE`, `W001-ROUTE`) resolve CI shared-fixture alias collisions with sequential reception tests while retaining explicit collision regression against a decoy sample having `labId: 'GTM-LAB1'`; verified `SUPER_ADMIN` unassigned queue aggregates 38 tasks (S005: 15, S004: 11, W001: 12) while lab manager is lab-scoped to 26; verified S005 resolves strictly to its own 15 tasks via canonical ID `GTM-LAB1`; verified primary action button renders `"Assign 15 unassigned task(s) to technician"` with ArrowRight SVG icon; verified real DOM interactions: method filter banner cleared, card clicked to navigate to sample route, and "Back to queue" clicked to navigate back to queue. Captured real browser CDP screenshot `sample_detail_manager_assign_route.png` and report `sample_detail_manager_assign_evidence.json`. | Manual assignment requires manager authority. | Released to Production (`7516f0e`); Follow-Up Route/Render Fix Implemented (Refs #119); Codex Verification and Closure Pending |
