...
| **#118** | 1 | QC batch inspection link disconnect, unhandled QC disposition, failed QC bypassing release gates | Reproduced & Implemented Locally | `1ed46ef` + follow-up | `qc_disposition_release_gate.test.js` (20/20), `qc_inspection_display_legacy_limits.test.js` (17/17), client production build (`vite build` clean in 7.18s), isolated Headless Chrome CDP browser journey (5/5 passed, `qc_inspection_legacy_reject_evidence.json`) | `7516f0e` | Canonical batch inspection returns actual control values, affected work items with canonical sample ID, original ID, lab ID, run profile, and notes. Unresolved failed QC strictly blocks sample approval (409) and report generation (409). Permitted disposition schema enforced; manager disposition override is atomic inside transaction, updates result flags while preserving non-QC invalidity flags (`MANUAL_INVALID`, `ORIGINALLY_INVALID`) and immutable released/superseded records; multi-step validity probe verified (originally invalid result stays invalid across FAIL -> PASS and PROCEED_WITH_WARNING); fails closed on DB errors; REANALYZE_BATCH updates work items to REANALYSIS_REQUIRED; audit.qc queue counts only unresolved batches. Follow-up display corrections: legacy `REJECT_REANALYSIS` mapped to prominent read-only banner (`REJECTED FOR RE-ANALYSIS (Legacy)`) with author, timestamp, justification, and dedicated read-only card; legacy `ACCEPT` kept neutral (`RECORDED LEGACY ACCEPT (Under Review)`, slate styling); factual audit history notices without sweeping ISO 17025 claims; editable form and old reason prefill strictly suppressed; blanks with missing limits render `"Not recorded"` (never 0, never implicit 0.05); blank status inference enforces strict `parseFiniteNumber` (accepts finite numbers, strings, and valid zero `0`, while rejecting empty/whitespace/false/array/non-finite inputs); explicit recorded statuses strictly preserved. Populated batch SSR and initial SSR verified. Verified in headless Chrome CDP with screenshots `qc_inspection_legacy_reject_modal.png` and `qc_inspection_legacy_reject_card.png`. | Historical released results and persisted decisions remain immutable. | Released to Production (`7516f0e`); Follow-Up Display Corrections Implemented; Codex Verification and Closure Pending |
| **#119** | 1 | Confusion between canonical, lab, and original IDs; 26-task fixture truncation; derived fractions vs ordered tasks; ReferenceError: ArrowRight is not defined on manager assignment route | Reproduced & Implemented Locally | `1ed46ef` + follow-up | `sample_assignment_identity.test.js` (8/8), `sample_assignment_manager_route.test.js` (9/9), client production build (`vite build` clean in 6.97s), isolated Headless Chrome CDP browser journey (6/6 passed, `sample_detail_manager_assign_evidence.json`) | `7516f0e` | Disambiguated canonical UUID (`sampleId`), sample lab ID (`labId`), and field ID (`originalId`); honest task pagination (26-task fixture exposes all 26 tasks with honest pagination metadata); bidirectional texture equivalence (`TEXTURE` satisfied by 3 fractions `SAND, SILT, CLAY`, and composite `TEXTURE` satisfies fractions); independent gate evaluation respecting SKIPPED/WAIVED; sampleWorkspace extracts linked QC batches to gate final approval; unassignedCount calculated across all work items; ManagerQueue routes assign to `?tab=work` and review to `?tab=review`. Prioritized canonical ID lookups (`findUnique({ where: { id } })`) before labId/originalId aliases to prevent cross-column collisions. Resolved live #119 blocker on deployed `7516f0e`: imported missing `ArrowRight` and `Eye` from `lucide-react` in `SampleDetail.jsx`; verified `SUPER_ADMIN` unassigned queue aggregates 38 tasks (S005: 15, S004: 11, W001: 12) while lab manager is lab-scoped to 26; verified S005 resolves strictly to its own 15 tasks via canonical ID `GTM-LAB1`; verified primary action button renders `"Assign 15 unassigned task(s) to technician"` with ArrowRight SVG icon; verified "Back to queue" preserves returnTo context. Captured real browser CDP screenshot `sample_detail_manager_assign_route.png` and report `sample_detail_manager_assign_evidence.json`. | Manual assignment requires manager authority. | Released to Production (`7516f0e`); Follow-Up Route/Render Fix Implemented (Refs #119); Codex Verification and Closure Pending |
| **#121** | 2 | Label print dialog issues, printable content isolation, title/filename handling | Reproduced & Implemented Locally | `1ed46ef` | `label_print.test.js` (5/5), local browser CDP evidence (`run_browser_evidence.cjs` exit code 0), client build (`vite build` clean in 7.26s) | `7516f0e` | Dual standard (101x54mm) and compact (50x25mm) thermal label formats; 100% offline QR code generation via `qrcode`; portal to `#label-print-portal` at `document.body` with print CSS isolation hiding all standard web UI; dynamic `document.title = Label-${sanitizedSampleId}` with `afterprint` restoration; autoPrint trigger support; intake rejection gate (`canPrintLabel: false`). Verified in local headless Chrome with standard (75,034 bytes) and compact (46,046 bytes) PDFs. | Browser-controlled print filename is best effort. Physical printer checks pending. | Released to Production (`7516f0e`); Codex Verification and Closure Pending |
...
- Test Suite: 9 passed, 9 total (clean execution via Jest in 1.535s):
  ...
  6. `[PASS]` `S005 also resolves accurately when queried via display lab code S005`: Workspace service fallback query by display lab ID returns exactly 15 tasks.
  7. `[PASS]` `S004 resolves strictly to its own 11 tasks`: Workspace service returns exactly 11 tasks.
  8. `[PASS]` `W001 resolves strictly to its own 12 tasks`: Workspace service returns exactly 12 tasks.
  9. `[PASS]` `Target URL uses canonical sampleId GTM-LAB1 and preserves returnTo queue context`: Validates target route `/samples/GTM-LAB1?tab=work&returnTo=%2Fmanager-queue%3Flane%3Dassign`.
...
- **Browser Journey Execution**: 6/6 steps passed (Exit 0):
  1. `[PASS]` `Step 1: Manager Queue Assign Lane Rendered`: Authenticated as `SUPER_ADMIN`, navigated to `/manager-queue?lane=assign`. Page loaded successfully, rendering S005, S004, W001, and aggregated 38 tasks.
  2. `[PASS]` `Step 2: SampleDetail loaded cleanly without Application Error`: Navigated to canonical route `http://127.0.0.1:<port>/samples/GTM-LAB1?tab=work&returnTo=%2Fmanager-queue%3Flane%3Dassign`. Verified DOM is free of `Application Error`, `ReferenceError`, and `ArrowRight is not defined`. Verified both canonical ID `GTM-LAB1` and display ID `S005` are present.
  3. `[PASS]` `Step 3: Primary Action Button rendered`: Button text verified as `"Assign 15 unassigned task(s) to technician"`, containing a valid rendered SVG icon (`<ArrowRight />`).
  4. `[PASS]` `Step 4: Back button text respects returnTo context`: Verified back button displays `"Back to queue"` (derived from `returnTo=%2Fmanager-queue%3Flane%3Dassign`).
  5. `[PASS]` `Step 5: S005 WorkItemsTable renders tasks count`: Verified table rows render S005's operational gates and wet chemistry analyses.
  6. `[PASS]` `Step 6: Evidence Artifacts Captured`:
