...
| **#121** | 2 | Label print dialog issues, printable content isolation, title/filename handling | Reproduced & Implemented Locally | `1ed46ef` | `label_print.test.js` (5/5), local browser CDP evidence (`run_browser_evidence.cjs` exit code 0), client build (`vite build` clean in 7.26s) | `7516f0e` | Dual standard (101x54mm) and compact (50x25mm) thermal label formats; 100% offline QR code generation via `qrcode`; portal to `#label-print-portal` at `document.body` with print CSS isolation hiding all standard web UI; dynamic `document.title = Label-${sanitizedSampleId}` with `afterprint` restoration; autoPrint trigger support; intake rejection gate (`canPrintLabel: false`). Verified in local headless Chrome with standard (75,034 bytes) and compact (46,046 bytes) PDFs. | Browser-controlled print filename is best effort. Physical printer checks pending. | Code LIVE in `v3.5.19`; Individual live acceptance and physical printer checks pending. OPEN |
| **#122** | 2 | LabMethods defaults failing or silently falling back across labs; URL scope ignored and stale-state cross-lab save corruption | Reproduced & Fixed Locally | Pending PR | `lab_method_defaults.test.js` (6/6 passed), React 18 mounted verification (`issue122-mounted-verification.cjs` 4/4 passed), client build (`vite build` clean in 9.15s) | `c5ed62e` (v3.5.19 base) | Scoped loading/saving for target lab; honors URL lab scope without silent fallback; clears and binds loaded defaults/error/wizard to target lab; prevents stale/error/loading state from enabling or dispatching saves (0 writes); out-of-order delayed response guard (`activeRequestIdRef`). Contract tests verify 403 cross-lab access, 403 unauthorized role, 400 mismatched methods, honest empty state. | Requires lab manager or global admin role. | Undergoing verification (Refs #122); 9 other shipped issues awaiting acceptance. Strictly OPEN |
...
- **Tests Executed**:
  ...
  - React 18 Mounted Synthetic Test Suite (`issue122-mounted-verification.cjs`, 4/4 suites passed):
...
- **Defects Reproduced at `c5ed62e`**:
  - Reproduced via `C:\Users\yigin\Documents\Codex\2026-09-21\se\work\issue122-mounted-review.cjs`:
...
- **Client Implementation (`client/src/pages/admin/LabMethods.jsx`)**:
  - Extracted URL scope via `getUrlLabId()` parsing `window.location.search` (`?labId=...`).
  - Initialized `selectedLabId` to `getUrlLabId() || user?.labId || ''`.
  ...
  - Added request sequence counter `activeRequestIdRef` to drop out-of-order delayed responses.
  ...
  - Reflects lab dropdown changes cleanly to URL query param via `window.history.replaceState` when available.
