...
async function runBenchmark() {
    ...
    const outPath = path.join(root, 'WP/project-management-audit-v1/independent-http-benchmark-results.json');
    fs.writeFileSync(outPath, JSON.stringify(reportData, null, 2));
    console.log(`\n✓ Benchmark results written to ${outPath}`);

    // Cleanup temp fixture
    try {
        // Independent run retains its isolated temporary fixture; no recursive deletion.
    } catch {}
