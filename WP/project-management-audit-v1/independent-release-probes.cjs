...
async function main(){
 ...
 const report={checkedAt:new Date().toISOString(),baseline:'e9e7acf5993a0ce21dc371ad69305c4220667584',fixtureDatabase:fixture,sourceHashBefore:before,sourceHashAfter:hash(),sourceUnchanged:before===hash(),cases:results.length,passed:results.filter(r=>r.passed).length,results};
 fs.writeFileSync(path.join(__dirname,'independent-release-results.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({cases:report.cases,passed:report.passed,sourceUnchanged:report.sourceUnchanged,results:results.map(({id,case:c,passed})=>({id,case:c,passed}))},null,2));
}
