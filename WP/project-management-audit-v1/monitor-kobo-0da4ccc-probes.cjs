...
async function main(){
 ...
 const report={checkedAt:new Date().toISOString(),baseline:'0da4ccc',sourceUnchanged:before===hash(),externalCallsMocked:externalCalls,results};
 fs.writeFileSync(path.join(__dirname,'monitor-kobo-0da4ccc-results.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
}
