...
async function runCase(viewport,route,locale){
 ...
 const shot=async label=>{const name=`independent-responsive-review14-${viewport.width}-${route.slice(1)}-${locale}-${label}.png`;await page.screenshot({path:path.join(__dirname,name)});(record.screenshots??=[]).push(name);};
 ...
 try{
  ...
  await page.goto(url);await page.locator('#collapseCoachBtn').waitFor();await page.waitForTimeout(180);
  record.actualLocale=await page.locator('#soilfer-tutorial-overlay .coach-top select').inputValue();
  record.renderedPause=await page.locator('#pause').innerText();record.renderedExit=await page.locator('#exit').innerText();
  record.localePass=record.actualLocale===locale&&record.renderedPause===expected.common.pause&&record.renderedExit===expected.common.exitGuide;
  await geometry('expanded',['collapseCoachBtn','pause','exit']);if(selectedShot)await shot('expanded');
  await clickCenter('pause');await page.locator('#resume').waitFor();await clickCenter('resume');await page.locator('#collapseCoachBtn').waitFor();
  await clickCenter('collapseCoachBtn');await geometry('collapsed',['expandCoachBtn','pauseCollapsedBtn','exitCollapsedBtn']);if(selectedShot)await shot('collapsed');
  await clickCenter('pauseCollapsedBtn');await page.locator('#resume').waitFor();await clickCenter('resume');await page.locator('#expandCoachBtn').waitFor();
  await clickCenter('expandCoachBtn');await page.locator('#exit').waitFor();await clickCenter('exit');await page.waitForTimeout(80);
  record.expandedExitRemoved=await page.locator('#soilfer-tutorial-overlay').count()===0;
  // New isolated local document solely to exercise collapsed Exit. No account handover is claimed here.
  await page.goto(url);await page.locator('#collapseCoachBtn').waitFor();await clickCenter('collapseCoachBtn');await clickCenter('exitCollapsedBtn');await page.waitForTimeout(80);
  record.collapsedExitRemoved=await page.locator('#soilfer-tutorial-overlay').count()===0;
  record.pass=record.localePass&&record.expandedExitRemoved&&record.collapsedExitRemoved&&!record.pageError&&record.states.every(s=>(!s.guide||(s.guide.x>=0&&s.guide.right<=viewport.width))&&(!s.coachScrollLeft)&&s.controls.every(c=>c.inViewport&&c.hitTarget&&!c.clipped));
...
(async()=>{
 ...
 const jobs=[];for(const viewport of [{width:320,height:700},{width:390,height:844},{width:700,height:360},{width:1440,height:1000}])for(const route of ['/login','/workbench'])for(const locale of ['en','es','es-419','fr','pt'])jobs.push(()=>runCase(viewport,route,locale));
