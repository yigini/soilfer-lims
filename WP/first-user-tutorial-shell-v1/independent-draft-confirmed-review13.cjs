// Bounded T10 companion: acknowledged input without further edits must not cause a confirmation loop.
...
// The real React-controlled Activity Receipts search lives outside the tutorial.
// It is a harmless app control, not a synthetic DOM injection or a laboratory result.
await p.getByRole('button',{name:/Activity Receipts/}).click();
const liveInput=p.getByPlaceholder('Search sample or receipt...');
await liveInput.fill('initial acknowledged input');
evidence.checks.initialControlledInput={value:await liveInput.inputValue(),outsideTutorial:await liveInput.evaluate(el=>!el.closest('[data-sf-tutorial]'))};
await p.locator('#sampleIdInput').fill('TEST-SAMPLE');
await p.locator('#viewSampleMapBtn').click();
await p.locator('#draftDiscardBtn').waitFor();
evidence.checks.initialConfirmation={dialog:await p.getByRole('alertdialog').count(),lookupStarted:heldOnce};
await p.locator('#draftDiscardBtn').click();await Promise.race([detailRequested,new Promise((_,rej)=>setTimeout(()=>rej(new Error('Sample detail GET not observed')),7000))]);
evidence.checks.pending={url:p.url().replace(base,''),detailHeld:heldOnce};await p.waitForTimeout(150);evidence.checks.unchangedControlledInput={value:await liveInput.inputValue(),dialog:await p.getByRole('alertdialog').count()};
evidence.checks.beforeResponse={url:p.url().replace(base,''),guideCount:await p.locator('#soilfer-tutorial-overlay').count(),session:await p.evaluate(()=>sessionStorage.getItem('soilfer_tutorial_v1'))};
releaseDetail();await p.waitForTimeout(1100);
evidence.checks.afterLateResponse={url:p.url().replace(base,''),guideCount:await p.locator('#soilfer-tutorial-overlay').count(),session:await p.evaluate(()=>sessionStorage.getItem('soilfer_tutorial_v1')!==null),dialog:await p.getByRole('alertdialog').count(),liveInputRemaining:await liveInput.count()};
evidence.passed=new URL(p.url()).pathname==='/samples/TEST-SAMPLE/map' && evidence.checks.afterLateResponse.dialog===0;
evidence.expected='An unchanged confirmed input must navigate once after the lookup without another confirmation.';evidence.limitation='Real Activity Receipts search control demonstrates the global draft guard behavior. It is not a numeric result editor; no clinical/laboratory draft loss or server persistence was tested.';
await c.close();
})().catch(e=>{evidence.harnessError=String(e.stack||e);process.exitCode=1;}).finally(async()=>{if(browser)await browser.close();await new Promise(r=>server.close(r));fs.writeFileSync(path.join(__dirname,'independent-draft-confirmed-review13.json'),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2));});
