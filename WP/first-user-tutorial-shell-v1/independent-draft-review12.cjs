// Bounded T10 test: fresh live input after draft confirmation and during awaited sample verification must require fresh confirmation.
...
evidence.checks.pending={url:p.url().replace(base,''),detailHeld:heldOnce};await liveInput.fill('fresh input entered while verification waits');await p.waitForTimeout(150);evidence.checks.freshControlledInput={value:await liveInput.inputValue(),dialog:await p.getByRole('alertdialog').count()};
...
evidence.reproduced=new URL(p.url()).pathname!=='/workbench' && evidence.checks.afterLateResponse.dialog===0;
evidence.expected='After confirming the initial dirty input, a new input event on a real app control during the awaited map lookup must require a fresh confirmation before navigation.';evidence.limitation='Real Activity Receipts search control demonstrates the global draft guard behavior. It is not a numeric result editor; no clinical/laboratory draft loss or server persistence was tested.';
await c.close();
})().catch(e=>{evidence.harnessError=String(e.stack||e);process.exitCode=1;}).finally(async()=>{if(browser)await browser.close();await new Promise(r=>server.close(r));fs.writeFileSync(path.join(__dirname,'independent-draft-review12.json'),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2));});
