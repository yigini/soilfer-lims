// Bounded regression: a tutorial sample GET must not navigate after Exit.
...
evidence.checks.pending={url:p.url().replace(base,''),detailHeld:heldOnce};await p.locator('#exit').click();await p.waitForTimeout(150);
evidence.checks.afterExitBeforeResponse={url:p.url().replace(base,''),guideCount:await p.locator('#soilfer-tutorial-overlay').count(),session:await p.evaluate(()=>sessionStorage.getItem('soilfer_tutorial_v1'))};
...
evidence.reproduced=evidence.checks.afterExitBeforeResponse.guideCount===0&&(new URL(p.url()).pathname!=='/workbench'||evidence.checks.afterLateResponse.guideCount>0);
evidence.expected='After Exit, the pending sample verification is canceled or ignored; route stays /workbench and guide stays absent.';
await c.close();
})().catch(e=>{evidence.harnessError=String(e.stack||e);process.exitCode=1;}).finally(async()=>{if(browser)await browser.close();await new Promise(r=>server.close(r));fs.writeFileSync(path.join(__dirname,'independent-map-race-review09.json'),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence,null,2));});
